...
def distill(args):
    ...
    teacher_model = RobertaForSequenceClassification.from_pretrained(args.teacher_model_path, num_labels=args.label_num, output_hidden_states=True).to(device)
    ...
    valid_dataset = TxtDataset2(data_path=f"/users/home/elenao23/2_finetuning_fix/data/{args.dataset_name}/validation.txt")
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if record >= np.max(metric_list):
            torch.save(student_model.state_dict(),
                    f"saved_models/distilled_spikformer/hyperparam_new_{args.temperature}_{args.max_length}_{args.dataset_name}_ic3_epoch{epoch}_{record}" +
