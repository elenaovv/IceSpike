...
from dataset import TxtDataset, TextDataset, ChnWikiDataset, IGCTextDataset
...
def train(args):
    ...
    teacher_model = RobertaModel.from_pretrained(args.teacher_model_path, num_labels=args.label_num, output_hidden_states=True).to(device)
    ...
    #corpus = load_dataset("mideind/icelandic-common-crawl-corpus-IC3", split="train")
    # # assert bookcorpus.features.type == wiki.features.type
    #raw_dataset = concatenate_datasets([bookcorpus, wiki])
    #raw_dataset = corpus
    #text_dataset = TextDataset(raw_dataset=raw_dataset)
    json_path = r'C:\Users\lenax\PycharmProjects\gigaword_scripts\extracted_texts.json'

    text_dataset = IGCTextDataset(json_path=json_path)
    ...
    for batch in tqdm(train_data_loader):
        ...
        if p > skip_p:
            continue
        train_iter += 1
        if train_iter % 8000 == 0:  # Checkpointing every 8000 iterations
            save_path = f"saved_models/checkpoint_iter_{train_iter}.pt"
